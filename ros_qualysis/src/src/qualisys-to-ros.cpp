  ...
  while (failure < 20 && ros::ok() && !success) {
    ...
    if (!crt_protocol_.Connected()) {
      if (!crt_protocol_.Connect(server_address_.c_str(), base_port_,
        ...
        ROS_ERROR_STREAM("TCP connection to " << server_address_ << ":" << base_port_ << " failed");
        ROS_ERROR_STREAM("Error: " << crt_protocol_.GetErrorString());
    ...
    if (!crt_protocol_.StreamFrames(CRTProtocol::EStreamRate::RateAllFrames, 0, 0, NULL,
...
void QualisysToRos::run() {
  ...
  if (!crt_protocol_.Connected()) {
    ROS_ERROR_THROTTLE(1, "Not connected to Qualisys server");
    return;
  }

  rt_packet_ = crt_protocol_.GetRTPacket();
  if (!rt_packet_) {
    ROS_ERROR_THROTTLE(1, "Failed to get RT packet");
    return;
  }
        ...
        if (body_count_ <= 0) {
          ...
          for (unsigned int i = 0; i < body_count_; i++) {
            if (rt_packet_->Get6DOFBody(i, px_, py_, pz_,
              ...
              if (std::isfinite(px_) && std::isfinite(py_) &&
                  std::isfinite(pz_) && std::isfinite(rotation_matrix_[0]) &&
